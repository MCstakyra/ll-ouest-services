 ...
 * Vérifie l'état d'authentification Firebase avant de charger les données.
...
export async function loadUserData() {
  try {
    ...
    if (!user || !token) {
      console.log('Aucun utilisateur Firebase ou token trouvé, utilisateur non connecté');
      clearUserCache();
      clearStoredToken();
      return null;
    }
    ...
    token = await refreshTokenIfNeeded(token, user);
    ...
    if (error.message.includes('Token invalide') || error.message.includes('expiré') || error.message.includes('invalid algorithm') || error.message.includes('Firebase Auth non initialisé')) {
      handleInvalidToken();
 ...
 */
async function refreshTokenIfNeeded(currentToken, user) {
  try {
    ...
    if (exp - now < CACHE_TTL) {
      ...
      const newFirebaseToken = await user.getIdToken(true); // Forcer le rafraîchissement
...
function updateAuthMenu(userData) {
  ...
  authContainer.innerHTML = `
    <div x-data="{ open: false }" @click.away="open = false" class="relative z-50">
      <button type="button" id="auth" class="auth-combined-button auth-dropdown-toggle" title="Authentification" @click="open = !open" :aria-expanded="open.toString()" aria-haspopup="true">
        ${
          isAuthenticated
            ? (userData.photoURL
                ? `<img src="${userData.photoURL}" alt="Photo de profil" class="w-8 h-8 rounded-xl mr-2 header-btn object-cover">`
                : `<div class="w-8 h-8 back-text rounded-xl overflow-hidden text-white flex items-center justify-center text-sm font-semibold mr-2">
                    ${initials}
                  </div>`)
            : `<div class="w-8 h-8 back-text rounded-xl bg-gradient-to-r from-ll-dark-green to-ll-dark-blue text-white flex items-center justify-center">
      ...
      <div 
        ...
        class="absolute header-btn right-0 mt-3 w-72 bg-ll-white dark:bg-ll-black rounded-xl shadow-lg-dark-custom p-4 border border-ll-light-gray/20 dark:border-ll-dark-blue/30 auth-menu-backdrop"
      >
        <div class="flex flex-col gap-2">
          <div class="flex items-center gap-3 pb-3 border-b border-ll-light-gray/30 dark:border-ll-dark-blue/50">
            ...
            <div>
              ...
              <p class="text-xs text-ll-text-gray font-Cinzel">${isAuthenticated ? userData.role : 'Invité'}</p>
              <p class="text-xs text-ll-text-gray font-Cinzel">${isAuthenticated ? userData.email : Date.now().toString()}</p>
            </div>
          </div>
          ${
            isAuthenticated
              ...
              <a href="/pages/user/profile.html" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-ll-blue/10 dark:hover:bg-ll-dark-blue/20 transition-all duration-150 auth-menu-item">
                ...
                <span class="text-sm font-Cinzel text-ll-black dark:text-ll-white">Mon Profil</span>
              </a>
              <a href="/pages/user/dashboard.html" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-ll-blue/10 dark:hover:bg-ll-dark-blue/20 transition-all duration-150 auth-menu-item">
                ...
                <span class="text-sm font-Cinzel text-ll-black dark:text-ll-white">Tableau de bord</span>
              </a>
              <a href="/pages/user/settings.html" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-ll-blue/10 dark:hover:bg-ll-dark-blue/20 transition-all duration-150 auth-menu-item">
                ...
                <span class="text-sm font-Cinzel text-ll-black dark:text-ll-white">Paramètres</span>
              </a>
              <a href="/pages/user/notifications.html" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-ll-blue/10 dark:hover:bg-ll-dark-blue/20 transition-all duration-150 auth-menu-item">
                ...
                <span class="text-sm font-Cinzel text-ll-black dark:text-ll-white">Notifications</span>
              </a>
              ...
              <a href="/pages/auth/signin.html" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-ll-blue/10 dark:hover:bg-ll-dark-blue/20 transition-all duration-150 auth-menu-item">
                ...
                <span class="text-sm font-Cinzel text-ll-black dark:text-ll-white">Se connecter</span>
              </a>
              <a href="/pages/auth/signup.html" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-ll-blue/10 dark:hover:bg-ll-dark-blue/20 transition-all duration-150 auth-menu-item">
                ...
                <span class="text-sm font-Cinzel text-ll-black dark:text-ll-white">S'inscrire</span>
              </a>
              <a href="/pages/auth/reset-password.html" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-ll-blue/10 dark:hover:bg-ll-dark-blue/20 transition-all duration-150 auth-menu-item">
                ...
                <span class="text-sm font-Cinzel text-ll-black dark:text-ll-white">Réinitialiser le mot de passe</span>
              </a>
              `
          }
        </div>
...
function updateDashboardWithUserData(userData) {
  // Implémenter la logique spécifique au dashboard si nécessaire
}
