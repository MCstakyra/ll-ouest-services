 ...
 * @param {string} reservationData.serviceName - Nom du service.
 * @param {string} reservationData.serviceCategory - Catégorie du service.
 ...
 * @param {string} reservationData.hour - Heure de réservation.
 * @param {string} reservationData.address - Adresse complète.
 * @param {string} reservationData.message - Message (1-1000 caractères).
...
function validateReservationData(reservationData) {
  const schema = {
    ...
    serviceName: { type: 'string', required: true, minLength: 2, maxLength: 100 },
    serviceCategory: { type: 'string', required: true, minLength: 2, maxLength: 50 },
    name: { type: 'string', required: true, minLength: 2, maxLength: 100, pattern: /^[a-zA-Z\s-]{2,100}$/ },
    phone: { type: 'string', required: false, pattern: /^\+33\s?\d{1,2}(\s?\d{2}){4}$/ },
    email: { type: 'string', required: true, pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, maxLength: 255 },
    date: { 
    ...
    hour: { type: 'string', required: true, minLength: 1, maxLength: 50 },
 ...
 * @param {string} reservationData.hour - Heure de réservation.
 * @param {string} reservationData.address - Adresse complète.
 * @param {string} reservationData.message - Instructions spéciales (1-1000 caractères).
...
async createReservation(reservationData) {
  try {
    ...
    const normalizedReservationData = {
      ...
      hour: reservationData.hour?.trim(),
      address: reservationData.address?.trim(),
    ...
    const templateData = {
      ...
      phone: normalizedReservationData.phone || 'Non renseigné',
      ...
      hour: normalizedReservationData.hour || 'Heure flexible',
      address: normalizedReservationData.address,
    ...
    const response = await apiFetch('/reservation', 'POST', {
 ...
 * @param {string} reservationData.hour - Heure de réservation.
 * @param {string} reservationData.address - Adresse complète.
 * @param {string} reservationData.message - Instructions (1-1000 caractères).
...
async updateReservation(id, reservationData) {
  try {
    ...
    validateReservationData(dataToValidate);
 ...
 * @param {Object} [filters={}] - Filtres pour la requête (ex. { serviceId, email, dateFrom, dateTo }).
...
async getAllReservations(page = 1, limit = 10, filters = {}) {
  try {
    ...
    // Ajouter les filtres valides
    const validFilters = ['serviceId', 'email', 'name', 'dateFrom', 'dateTo', 'serviceCategory', 'status'];
